...
const createOrderHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === 'POST') {
    try {
      const { quoteId, startDate, endDate, totalAmount, description } = req.body;

      // Validation checks to ensure required fields are present
      if (!quoteId || !startDate || !endDate || !totalAmount || !description) {
